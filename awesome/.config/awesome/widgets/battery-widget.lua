...
function battery_widget:discover()
	...
	for adapter in io.popen("ls -1 " .. pow):lines() do
		----------- TO IGNORE IPHONE WHEN CONNECTED -----------
		if not (adapter == "apple_mfi_fastcharge") then
			local type = read_trim(pow .. adapter .. "/type")
			table.insert(adapters[type], adapter)
		end
		-------------------------------------------------------
		---------- DON'T IGNORE IPHONE WHEN CONNECTED ----------
		-- local type = read_trim(pow .. adapter .. "/type")
		-- table.insert(adapters[type], adapter)
		--------------------------------------------------------
	end
...
function battery_widget:get_state()
	...
	local sysfs = (
		file_exists(bat .. "/" .. sysfs_names.charging.rate) and sysfs_names.charging or sysfs_names.discharging
	)
...
function battery_widget:update()
	...
	if naughty then
		if ctx.state == "discharging" and ctx.percent and ctx.percent <= self.alert_threshold then
			self:notify(substitute(self.alert_title, ctx), substitute(self.alert_text, ctx))
